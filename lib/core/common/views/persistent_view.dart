...
class _PersistentViewState extends State<PersistentView>
  ...
  /// In the `build` method, it checks if a custom `body` widget is provided. If not,
  /// it uses `context.watch<TabNavigator>().currentPage.child` to access the child
  /// widget of the current `TabNavigator`. This effectively displays the content
  ...
  Widget build(BuildContext context) {
    ...
    /// widget to the `PersistentView`. If not provided, it defaults to using the
    /// `child` of the current `TabNavigator`.
    return widget.body ?? context.watch<TabNavigator>().currentPage.child;
  }

  /// This method is overridden to return `true`. This tells Flutter to keep the state
  /// of this widget alive even when it's not currently visible on the screen.
  ///
